# mboy (miraeboy CLI) Windows installer
#
# 사용법:
#   irm https://raw.githubusercontent.com/corbie79/miraeboy/main/install-cli.ps1 | iex
#   iex "& { $(irm .../install-cli.ps1) } --Version v1.2.0"
#   iex "& { $(irm .../install-cli.ps1) } --InstallDir $env:USERPROFILE\.local\bin"
#
# 환경변수:
...
$TmpDir = Join-Path $env:TEMP "mboy-install-$([System.Guid]::NewGuid().ToString('N').Substring(0,8))"
New-Item -ItemType Directory -Path $TmpDir | Out-Null

try {
    Write-Host "==> Installing mboy $Version (windows/$Arch)"
    ...
    $Dest = Join-Path $InstallDir "mboy.exe"
    ...
    Write-Host ""
    Write-Host "  OK  mboy $Version installed -> $Dest" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Quick start:"
    Write-Host "    mboy login --server http://miraeboy.example.com:9300"
    Write-Host "    mboy repo list"
